        ...
        <div class="card col shadow-none p-4 mb-4 text-center">
            ...
            <div class="card-footer text-center">
                <div class="btn-group">
                    ...
                    __tr('Help') }}</a>
                    @endif
                     <a class="lw-btn btn btn-secondary btn-sm" href="{{ route('central.addons.read.list') }}">{{
                    __tr('Back to Addons') }}</a>
                </div>
            </div>
        </div>
	...
	<strong><?= __tr('Last verified on') ?></strong> <br> <?= formatDate($addonLicInfo('registered_at')) ?> (<?= formatDiffForHumans($addonLicInfo('registered_at')) ?>)
        ...
        <div class="mt-3">
            <?php
                ...
                $datetime = $addonLicInfo('supported_until');
				$dateInAppTimezone = null;
                if($datetime and !(Str::contains($datetime, ' '))) {
                    try {
                    $date = Carbon\Carbon::parse($datetime);
                    $dateInAppTimezone = $date->setTimezone($addonLicInfo('timezone') ?: config('app.timezone', 'Asia/Jakarta'));
            ...
            @if ($datetime and $dateInAppTimezone)
                @if ($dateInAppTimezone->lte(now()))
                    ...
                    <h2>{{ $formattedDate }}</h2>
                     <a target="_blank" href="https://codecanyon.net/checkout/from_item/55959195?support=renew_6month">{{  __tr('Click here to renew your support') }}</a>
                @endif
            @endif
		</div>
	</div>
